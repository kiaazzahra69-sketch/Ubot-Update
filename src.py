...
from config import (BLACKLIST_KATA, LOG_SELLER, LOG_PELER, OWNER_ID, WAJIB_JOIN,
                    costum_font)
    ...
    if list_error:
        for data in list_error:
            ...
            await bot.send_message(
                LOG_SELLER,
                f"<blockquote><b>Userbot {ubot} failed to start due to {reason}, deleted user on database</b></blockquote>",
            )
            try:
                await bot.send_message(
                    int(ubot), f"<blockquote><b>Userbot anda telah dihapus karna {reason}.</b></blockquote>"
                )
            except Exception:
                pass
...
async def start_main_bot():
    ...
    # Kirim ke LOG_SELLER
    try:
        await bot.send_message(LOG_PELER, f"<blockquote>{message}</blockquote>")
    except Exception:
        pass
